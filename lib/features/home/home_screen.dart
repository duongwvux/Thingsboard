...
class HomeScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: AppAsyncView<List<Dashboard>>(
        value: dashboardsAsync,
        loading: const _DashboardListSkeleton(),
        onRetry: () => ref.read(dashboardsProvider.notifier).refresh(),
        dataBuilder: (dashboards) {
          if (dashboards.isEmpty) {
            return const AppEmptyState(
              icon: Icons.dashboard_outlined,
              title: 'Chưa có dashboard nào',
              description: 'Tạo dashboard trong ThingsBoard rồi quay lại',
            );
          }
  ...
  void _openDashboard(
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) =>
            DashboardDetailScreen(dashboard: dashboard, token: token),
      ),
    );
  }
}
...
class DashboardDetailScreen extends StatefulWidget {
  ...
  const DashboardDetailScreen({super.key, required this.dashboard, this.token});
...
class _DashboardDetailScreenState extends State<DashboardDetailScreen> {
  ...
  void _initWebView() {
    ...
    final embedUrl =
        '$baseUrl/dashboard/$dashId?accessToken=$token'
        '&hideToolbar=true';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.dashboard.title, overflow: TextOverflow.ellipsis),
        actions: [
          // Nút reload
          IconButton(
