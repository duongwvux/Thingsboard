...
class TbRpcService {
  ...
  Future<bool> sendLedRpcRequest(String deviceId, bool isOn) async {
    try {
      ...
      print('❌ Lỗi gọi RPC từ Flutter: $e');
      return false;
    }
  }
  ...
  Future<bool?> getLatestLedState(String deviceId) async {
    try {
      ...
      final String url = '/api/plugins/telemetry/DEVICE/$deviceId/values/timeseries';
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        if (data.containsKey('ledState') && data['ledState'].isNotEmpty) {
          ...
          return latestValue == 'ON' || latestValue == 'true' || latestValue == true || latestValue == 1;
        }
      }
    } catch (e) {
      print('❌ Lỗi khi lấy trạng thái ban đầu của LED: $e');
    }
